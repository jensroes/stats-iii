...
## Fitting a normal linear model using `lm`: exercise 

- Work through scripts:
  - `exercises/calculating_sigma.R`
  - `exercises/interpreting_coefficients_1.R`
  - `exercises/interpreting_coefficients_2.R`

- For the first script (exercise), you will need this:
...
- Calculate *t* and *p*-value for the coefficient for age under the assumption that the true value for $\beta_\text{age}$ is 0.0.
...
- Open `exercises/confidence_interval.R` and complete the script,
...
mutate(sims, 
  ...
  scale_colour_manual(values = c("no" = "red", "yes" =" grey50")) +
...
- Normal model assumes that data are continues normal distributed.
- Estimate unknown population parameters using MLE.
- For rts researchers often use log-normal distributions to correct for positive skew.
...
- Next week: how can we compare different models of the data?
...
- Familiarise yourself with [the demo](https://now.ntu.ac.uk/d2l/le/content/887088/viewContent/9653011/View) for the formative assessment.
