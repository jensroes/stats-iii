...
- Value of $\mu_i$ is deterministic ("$=$") linear function of values of predictor variables $x$ for each observation $i$ and the unknowns $\beta_0$ (intercept) and $\beta_1$ (slope):
  ...
  - Hence, $\beta_1$ is the difference between $x_{1i}=1$ and $x_{1i}=0$: what do we need to add to $\beta_0$ when $x_{1i}=1$ instead of $x_{1i}=0$.
</div>
...
\mu_i &= \beta_0 + \beta_\text{age} \cdot x_{\text{age}[i]} + \beta_\text{sex} \cdot x_{\text{sex}[i]}\\
&\text{for } i \in 1\dots n
\end{align}
$$
