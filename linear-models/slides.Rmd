...
- Formally describe a statistical model
- Implement statistical models (continuous, categorical predictors)
- Interpret model coefficients and quantities related to null hypothesis testing (confidence interval, *t*-value, *p*-value)
- Conceptual understand how maximum likelihood estimation works (how `lm` determines model coefficients)
- Predict hypothetical outcomes given a statistical model (to visualise results, calculating residual error and therefore log-likelihood and model comparison)
...
>- Extract the `linear-models` and `model-comparison` folder
>- Move these into last week's directory
...
>- We assume that the logarithm of the $rt$ denoted as $\text{logrt}_1, \text{logrt}_2\dots \text{logrt}_n$, $\text{ for } i \in 1\dots n$ is sampled from a log-normal distribution with a fixed and unknown mean $\mu$ and standard deviation $\sigma$.
...
> For $K = 2$ we can predict rts for a hypothetical female age 95.
...
> With sexes denoted by $\text{sex}_{1},x_{2}\dots \text{sex}_{i}\dots \text{sex}_{n}$ and age by $\text{age}_{1},\text{age}_{2}\dots \text{age}_{i}\dots \text{age}_{n}$ $\text{ for } i \in 1\dots n$.
...
\beta_0 + \beta_\text{sex} \cdot \text{sex}_{i} + \beta_\text{age} \cdot \text{age}_{i}, \text{ if sex}_i = 1\texttt{ [male]}
\end{array}
\right.
$$
