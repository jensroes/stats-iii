...
- Understand the concept of parsimony
...
> In other words, knowing the RSS allows you to calculae the log-likelihood.
...
>- Adding the predictor $\text{sex}$ in $\mathcal{M}_1$ does not contribute to the variance in the data explained by model $\mathcal{M}_0$.
...
- with the *F*-statistic being distributed with $K$ and $N-K-1$ degrees of freedom.
- We can compare two models $\mathcal{M}_1$ and $\mathcal{M}_0$ with $K_1$ and $K_0$ predictors, respectively, and where $K_0 < K_1$ and all the $K_0$ predictors in $\mathcal{M}_0$ are also presented in $\mathcal{M}_1$, i.e. $\mathcal{M}_0$ is *nested* in $\mathcal{M}_1$.
...
>- TSS of `model_0`: in model with no predictors, variance of outcome and residuals are identical. 
