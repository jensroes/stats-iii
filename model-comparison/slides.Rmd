...
- Evaluating models based on likelihood.
- Evaluating models based on residuals.
- Comparing models based on fit to data.
...
## Why do we need model comparions 
...
- Philosopher William of Ockham (1285-1347/49): "pluralitas non est ponenda sine necessitate"
...
- Let's denote these log likelihoods as log $\mathcal{L}_0$ and log $\mathcal{L}_1$; their ratio is:
...
- Is the reason in the data or in the model of the data?
- How can we reduce the residual deviation?
...
- Coefficient of determination: ability of a model to predict the outcome variable.
...
- $R^2$: proportion of total variation explained by the variation in the predictor variable(s).
...
## Model comparisons made easy {.smaller}
...
- Models with different probability models (e.g. Poisson vs negative biomial).
- More about model comparisons for generalised linear models, see [https://cran.r-project.org/web/packages/fic/vignettes/fic.pdf](this manual). 
