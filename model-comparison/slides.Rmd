...
is the ratio of ESS (explained sum of squares) and TSS (total sum of squares) for which we need to work out the RSS (residual sum of squares; based on residuals):
...
```{r echo = F}
# Compare models
print(anova(model_1, model_2, model_3, model_4), signif.stars=F)
```
</div>
...
drop1(model_4, scope = ~ sex * age, test = 'F')
```

```{r echo = F}
# Compare models
print(drop1(model_4, scope = ~ sex * age, test = 'F'), signif.stars=F)
```
