...
## How do you trust someone's results?
...
- It is difficult to relate code to figures, tables, numbers etc. especially where results were copy-pasted from the stats software.
...
- RMarkdown is the best way of doing this using R (in combination with *knitr*, *pandoc*, $\LaTeX$).
...
- Note that this is an R-project file with sub-directories:
...
>- RStudio comes with necessary R packages `rmarkdown` (and `pandoc`), `knitr` 
>- To create pdf outputs, $\LaTeX$ must be installed which gives you access to a wide range of $\LaTeX$ tools.
>- However we only require the package `tinytex`
...
- YAML (Y*AML* A*in’t* *a* M*arkup* L*anguage*) is a minimal markup language.

## The setup chunk: global options used by `knitr`

>- Note ````{r setup, echo=FALSE}`
>- `setup` is a label of this chunk (optional; useful for navigation and cross-referencing of figures and tables; must be unique).
>- Chunk configuration option `echo = FALSE`: don't display chunk in output; `echo = TRUE`: display chunk.
>- Add this code to the setup chunk:
...
- How about $\sigma_e^2$?
- For a list of $\LaTeX$ symbols [see page 19 in lecture notes](https://now.ntu.ac.uk/d2l/le/content/961441/viewContent/12100893/View).
...
>- "APA article (6th edition)" template using `papaja`
>- APA 7 edition available too

## The end

>- You will use RMarkdown for your class assignment (get started now).
>- Read the [lecture notes](https://now.ntu.ac.uk/d2l/le/content/961441/viewContent/12100893/View) on rmarkdown.
