...
## How do you know you can trust someones published results? {.smaller}
...
## Minimal RMarkdown example {.smaller}
...
- Notice the YAML header between `---` (Y*AML* A*in’t* *a* M*arkup* L*anguag*e*)
...
- Top tip: you can literally copy $\LaTeX$ for formulas from Wikipedia: find the formula for the Gaussian probability density function.
...
- Cross-reference figure using `\ref{fig:scatterplot}` in the text (for tables use `\ref{tab:descriptives}`). 
- You might have noticed that the figure changed position. $\LaTeX$ is calling objects like figures and tables *floats* cause the placement depends on constraints such as whitespace on page, number of figures on page ...
...
- Also label you table chunk `smoker` and cross-reference the table in the text like so `\ref{tab:smoker}`.
...
- `$x + y$`, `$x - y$`
- Multiplication use either `$\cdot$` or `$\times$` to get $\cdot$ or $\times$, respectively.
